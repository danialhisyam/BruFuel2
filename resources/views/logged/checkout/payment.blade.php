        ...
        <div id="slideWrapper" class="slide-wrapper">
            ...
            >
                <span 
                    ...
                    "
                >
                    {{ session('checkout.vehicle.make') ?? '' }} {{ session('checkout.vehicle.model') ?? '' }}
                </span>
            </div>
            ...
            <!-- ✅ Fuel Amount Dropdown (text nudged left) -->
            <div 
                style="
                    position:absolute;
                    left:218px;
            ...
            >
                <select 
                    ...
                    style="
                        ...
                        padding-left:40px;
                        text-align:left;
                        color:{{ session('checkout.payment.amount') ? 'white' : 'rgba(177,177,177,0.3)' }};
                        font-family:'Poppins';
                        font-size:12px;
                        font-weight:500;
                        appearance:none;
                    ...
                    "
                    onchange="
                        this.style.color = this.value ? 'white' : 'rgba(177,177,177,0.3)';
                        updateFuelAmountDisplay(this);" onchange="updateFuelAmountDisplay(this)">

                    
                    <option value="" disabled {{ !session('checkout.payment.amount') ? 'selected' : '' }}>Select Amount</option>
                    <option value="5" {{ session('checkout.payment.amount') == '5' ? 'selected' : '' }}>B$5</option>
                    <option value="10" {{ session('checkout.payment.amount') == '10' ? 'selected' : '' }}>B$10</option>
                    <option value="15" {{ session('checkout.payment.amount') == '15' ? 'selected' : '' }}>B$15</option>
                    <option value="20" {{ session('checkout.payment.amount') == '20' ? 'selected' : '' }}>B$20</option>
                    <option value="30" {{ session('checkout.payment.amount') == '30' ? 'selected' : '' }}>B$30</option>
                    <option value="40" {{ session('checkout.payment.amount') == '40' ? 'selected' : '' }}>B$40</option>
                    <option value="50" {{ session('checkout.payment.amount') == '50' ? 'selected' : '' }}>B$50</option>
                </select>
            </div>
            ...
            opacity: 0;
            transform: translateY(40px);
            pointer-events: none; /* 🧩 Can't block anything while hidden */
            }

            #paymentPopup.show {
            opacity: 1 !important;
            transform: translate(-50%, -50%) !important;
            pointer-events: auto !important; /* 🧩 Now clickable when shown */
            }
           ...
           tracking-widest" />
            ...
            <div id="selectedFuelDisplay" data-layer="B$40" class="B40" 
                style="width: 63px; height: 20px; left: 318px; top: 791px; position: absolute; 
                        justify-content: center; display: flex; flex-direction: column; 
                        color: white; font-size: 24px; font-family: Poppins; 
                        font-weight: 600; word-wrap: break-word">
                B$00
            </div>

            <script>
                function updateFuelAmountDisplay(selectElement) {
                    const display = document.getElementById("selectedFuelDisplay");
                    const value = selectElement.value;
                    display.textContent = value ? `B$${value}` : "B$0";
                }
            </script>
...
function getTotal() {
    const base = parseFloat(amountSelect.value) || 0;
    return base + 5;
}

// 🧩 Update visual button & text
function updateButton(label) {
    const total = getTotal();
    confirmText.textContent = `${label} (B$${total})`;
}

// 🧩 Validation for card payment
function checkInputs() {
    const allFilled = Array.from(inputs).every(i => i.value.trim() !== "");
    const amountChosen = amountSelect && amountSelect.value !== "";

    if (allFilled && amountChosen) {
        ...
        confirmText.style.opacity = "0.5";
            ...
            function moveCircle(topValue) {
                ...
                if (topValue === 409) {
                    // 💳 Card mode
                    updateButton("PAY NOW");
                    checkInputs();
                } else if (topValue === 655) {
                    // 💵 Cash mode
                    updateButton("PAY BY CASH");
                    confirmBtn.style.background = "#760000";
                    confirmBtn.style.cursor = "pointer";
                    confirmText.style.opacity = "1";
                }
            ...
            // 🧩 Event listeners
            topGroup.addEventListener('click', () => moveCircle(409));
            bottomGroup.addEventListener('click', () => moveCircle(655));
            inputs.forEach(i => i.addEventListener('input', checkInputs));
            amountSelect.addEventListener('change', () => {
                updateButton(ellipse3.style.top === "655px" ? "PAY BY CASH" : "PAY NOW");
                if (ellipse3.style.top === "409px") checkInputs();
            });
            </script>


            <script>
            /* ✅ Card Number – groups every 4 digits */
            ...
            document.querySelectorAll('input[placeholder="Muhammad Ali"]').forEach(input => {
            input.addEventListener('input', e => {
                e.target.value = e.target.value
            ...
            document.querySelectorAll('input[placeholder="01/25"]').forEach(input => {
            input.addEventListener('input', e => {
                e.target.value = e.target.value
                .replace(/\D/g, '')             // digits only
            ...
            document.addEventListener("DOMContentLoaded", () => {
                ...
                const totalDisplay = document.getElementById('selectedFuelDisplay');

                function updateFuelAmountDisplay(selectElement) {
                    const base = parseFloat(selectElement.value);
                    const total = (isNaN(base) ? 0 : base) + 5;

                    // ✅ update button text
                    confirmText.textContent = `PAY NOW (B$${total})`;

                    // ✅ keep top display only base
                    totalDisplay.textContent = base ? `B$${base}` : "B$0";
                ...
                function checkInputs() {
                    ...
                    if (allFilled && amountChosen) {
                        ...
                        confirmBtn.style.cursor = "pointer";
                        confirmText.style.opacity = "1";
                        confirmBtn.onclick = null; // popup handles it later
                    } else {
                        confirmBtn.style.background = "#4B5563";
                        confirmBtn.style.cursor = "not-allowed";
                        confirmText.style.opacity = "0.5";
                ...
                // ✅ event listeners outside of function
                inputs.forEach(i => i.addEventListener("input", checkInputs));
                amountSelect.addEventListener("change", e => updateFuelAmountDisplay(e.target));
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    // 🧩 Popup text fields
    const refNumber = popup.querySelector("div[style*='top:232px'][style*='text-align:right']");
    const paymentMethod = popup.querySelector("div[style*='top:257px'][style*='text-align:right']");
    const paymentTime = popup.querySelector("div[style*='top:281px'][style*='text-align:right']");
    const senderName = popup.querySelector("div[style*='top:306px'][style*='text-align:right']");
    ...
    function getCurrentTime() {
        const now = new Date();
        return now.toTimeString().split(" ")[0].slice(0, 8); // HH:MM:SS
    }

    function generateRef() {
        return Math.floor(10000000 + Math.random() * 90000000); // 8-digit random
    }

    // ✅ When "PAY NOW"/"PAY BY CASH" is clicked → show popup
    confirmBtn.addEventListener("click", () => {
        if (confirmBtn.style.background === "rgb(118, 0, 0)") {
            const base = parseFloat(amountSelect.value) || 0;
            const total = base + 5;
            const method = ellipse3.style.top === "655px" ? "Cash" : "Credit Card";
            const nameInput = document.querySelector('input[placeholder="Muhammad Ali"]');

            popupTotal.textContent = `B$${total}`;
            refNumber.textContent = generateRef();
            paymentMethod.textContent = method;
            paymentTime.textContent = getCurrentTime();
    ...
    continueBtn.addEventListener("click", () => {
        ...
        <div id="paymentPopup"
            style="
                ...
                z-index:999;
            ">
        <div style="width:351px;height:513px;position:relative;">
            <img src='/images/success.png'
                style="width:113px;height:113px;left:119px;top:33px;position:absolute;" />
            <div style="width:351px;text-align:center;color:#F4E5E5;
                        font-size:20px;font-family:'Poppins';font-weight:700;
                        position:absolute;top:154px;">Payment Success!</div>
            <div style="width:351px;text-align:center;color:#C8C2C2;
                        opacity:0.6;font-size:12px;font-family:'Poppins';
                        position:absolute;top:187px;">Your payment has been successfully done.</div>
          
            <div data-layer="Line 3" class="Line3"
                style="
                    position: absolute;
                    top: 220px; /* ⬅ about 23px below your 187px text */
                    left: 50%;
                    transform: translateX(-50%);
                    width: 280px; /* moderate width centered */
                    height: 0;
          ...
          <div style="left:35px;top:232px;position:absolute;color:#F4E5E5;font-size:10px;">Ref Number</div>
            ...
            <div style="width:310px;height:47px;left:21px;top:445px;position:absolute;
                        background:#770000;border-radius:33px;"></div>
            <div style="width:236px;height:30px;left:58px;top:453px;position:absolute;
                        text-align:center;display:flex;align-items:center;justify-content:center;
                        color:white;font-size:16px;font-family:'Poppins';font-weight:700;cursor:pointer;">
                CONTINUE
            </div>
        </div>
        </div>
